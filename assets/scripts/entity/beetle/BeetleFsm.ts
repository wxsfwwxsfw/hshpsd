...
import BeetleFsmForIdle from './BeetleFsmForIdle';
import BeetleFsmForAttack from './BeetleFsmForAttack';
import BeetleFsmForMove from './BeetleFsmForMove';

export default class BeetleFsm extends FiniteStateMachine {
    ...
    initAnimEvent(){
        this.anim.on(cc.Animation.EventType.FINISHED, ()=>{
            ...
            if(whiteList.some(item => clipName.includes(item))){
                const beetle = this.node.getComponent(Beetle)
                beetle.state = ENTITY_STATE_ENUM.IDLE
            }
        }, this)
    }
    ...
    initStateMachines(){
        this.stateMachines.set(ENTITY_STATE_ENUM.IDLE, new BeetleFsmForIdle(this))
        this.stateMachines.set(ENTITY_STATE_ENUM.MOVE, new BeetleFsmForMove(this))
        this.stateMachines.set(ENTITY_STATE_ENUM.DIE, new FiniteState(this, {path: 'beetle/die/die'}))
        this.stateMachines.set(ENTITY_STATE_ENUM.ATTACK, new BeetleFsmForAttack(this))
