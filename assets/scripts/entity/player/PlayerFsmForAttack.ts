...
import FiniteState, { ANIM_SAMPLE, ANIM_SPEED } from '../../anim/FiniteState';
import { getEnumfromEntityDirectionIndex } from '../../Util';

export default class PlayerFsmForAttack extends FiniteStateMachineSub {

    constructor(fsm: FiniteStateMachine) {
        super(fsm)

        this.stateMachines.set(ENTITY_DIRECTION_ENUM.LEFT, new FiniteState(fsm, { path: 'player/hero', mode: cc.WrapMode.Loop, resourceType: RESOURCE_TYPE_ENUM.SPINE, entityState: ENTITY_STATE_ENUM.ATTACK, speed: 1.2 }))
        this.stateMachines.set(ENTITY_DIRECTION_ENUM.RIGHT, new FiniteState(fsm, { path: 'player/hero', mode: cc.WrapMode.Loop, resourceType: RESOURCE_TYPE_ENUM.SPINE, entityState: ENTITY_STATE_ENUM.ATTACK, speed: 1.2, mirror: true }))
    }
