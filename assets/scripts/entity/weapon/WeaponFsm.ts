...
export default class WeaponFsm extends FiniteStateMachine {
    ...
    run(): void {
        const weapon = this.node.getComponent(Weapon)
        switch(this.currentState){
            ...
            break
            default:
                this.currentState = this.stateMachines.get(weapon.state)
            break
        }
    }
    ...
    initStateMachines(){
        this.stateMachines.set(ENTITY_STATE_ENUM.IDLE, new FiniteState(this, {path:'weapon/idle', resourceType: RESOURCE_TYPE_ENUM.SPRITE_FRAME}))
