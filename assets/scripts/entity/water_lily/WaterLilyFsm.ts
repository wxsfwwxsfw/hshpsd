...
export default class WaterLilyFsm extends FiniteStateMachine {
    ...
    run(): void {
        const lily = this.node.getComponent(WaterLily)
        switch(this.currentState){
            ...
            break
            default:
                this.currentState = this.stateMachines.get(lily.state)
            break
        }
    }
    ...
    initStateMachines(){
        this.stateMachines.set(ENTITY_STATE_ENUM.IDLE, new FiniteState(this, {path:'water_lily/water_lily', resourceType: RESOURCE_TYPE_ENUM.SPRITE_FRAME}))
    }
}
