...
const map = [
    ...
    [
        ...
        { type: TILE_TYPE_ENUM.FLOOR, name: 'f_1_1' },  // y=1 地刺T4
    ...
    [
        ...
        { type: TILE_TYPE_ENUM.FLOOR, name: 'f_1_2' },  // y=2 草Ca（挡SA并控桃A落(2,3)）
        { type: TILE_TYPE_ENUM.FLOOR, name: 'f_0_1' },  // y=3 地刺T1
        { type: TILE_TYPE_ENUM.FLOOR, name: 'f_0_1' },  // y=4 蜘蛛C H↑（草Ca挡其上射）
    ...
    [
        ...
        { type: TILE_TYPE_ENUM.FLOOR, name: 'f_1_2' },  // y=5 桃子C（站原地LEFT投？）
        { type: TILE_TYPE_ENUM.WALL, name: 'w_down_0' },
    ],
    [
        ...
        { type: TILE_TYPE_ENUM.FLOOR, name: 'f_1_3' },  // y=3 蜘蛛B H←（桃B遮(4,3)）
...
// SA(1,2)RIGHT：草(2,2)挡（SA射RIGHT→(2,2)=grass→stop，(3,2)以后安全）
// SC(2,4)UP：草(2,2)挡SC上射（SC→(2,3)(2,2)=grass→stop，(2,1)安全）
//   草(2,2)同时挡SA和SC ✓
// SB(5,3)LEFT：桃B(4,4)→原地UP投→草(4,2)→桃停(4,3)→遮SB ✓
// SD(5,2)DOWN：威胁(5,3)(5,4)(5,5)，取钥须绕(4,1) ✓
// SE(3,4)UP：石块(3,2)挡（SE→(3,3)(3,2)=stone→stop，(3,1)安全）
const spiders = [
...
// 桃A(1,5)：站(2,4)UP投→草(2,2)→桃停(2,3)→遮SA y=2（桃落y=3不是y=2！SA在y=2，桃在y=3遮不了SA）
// 问题：SA在y=2 RIGHT，草(2,2)已经挡住了射线，不需要桃子遮SA
// 桃A(1,5)重新用途：遮y=3行某蜘蛛，或作为开关旁诱饵
// 实际：桃A(1,5)→站(2,4)UP投→草(2,2)→桃停(2,3)→(2,3)有桃，可遮SC(2,4)射出的(2,3)
//   但SC已被草(2,2)挡住了...桃A的确是多余的。
//   改：桃A用于遮挡另一个方向的威胁，或作为开关触发条件（某些机制？）
//   简化：只用两个桃子，桃B遮SB即可。把桃A改为地图装饰或删除，改为只留桃B和桃C
const peaches = [
...
// G1(3,2)冲突石块→改G1(2,1)；G2(3,1)
const floor_traps = [
    { type: ENTITY_TYPE_ENUM.FLOOR_TRAP, x: 2, y: 1, dir: ENTITY_DIRECTION_ENUM.DOWN, state: ENTITY_STATE_ENUM.ATTACK, entityId: 'l37_ft_1' },
    { type: ENTITY_TYPE_ENUM.FLOOR_TRAP, x: 3, y: 1, dir: ENTITY_DIRECTION_ENUM.DOWN, state: ENTITY_STATE_ENUM.ATTACK, entityId: 'l37_ft_2' },
]
const floor_trap_switches = [
    { type: ENTITY_TYPE_ENUM.FLOOR_TRAP_SWITCH, x: 1, y: 4, dir: ENTITY_DIRECTION_ENUM.DOWN, state: ENTITY_STATE_ENUM.IDLE, entityId: 'l37_ft_1' },
    { type: ENTITY_TYPE_ENUM.FLOOR_TRAP_SWITCH, x: 1, y: 3, dir: ENTITY_DIRECTION_ENUM.DOWN, state: ENTITY_STATE_ENUM.IDLE, entityId: 'l37_ft_2' },
]
