...
const map = [
    ...
    [
        ...
        { type: TILE_TYPE_ENUM.FLOOR, name: 'f_0_1' },  // y=1
        { type: TILE_TYPE_ENUM.FLOOR, name: 'f_1_2' },  // y=2 草Cb（桃B停(4,3)）
...
// SA x=1,y=3 RIGHT：桃A落(2,3)遮挡 → x=3..5,y=3安全
// SB x=5,y=3 LEFT：桃B落(4,3)遮挡 → x=1..3,y=3安全
// SC x=5,y=2 DOWN：威胁(5,3)(5,4)(5,5)；钥匙在(5,1)安全（SC上方）
// 双桃均遮后，x=3,y=3（有双孔地刺）仍需小心，但已可穿越
const spiders = [
...
// 桃A x=1,y=5：拾取后站(2,4)向UP投，草(2,2)使桃停(2,3)，遮SA
// 桃B x=4,y=4：站(4,4)向UP投，草(4,2)使桃停(4,3)，遮SB
// 顺序要求：先遮SA（需到左侧），再遮SB（需到右侧），最后踩双开关
const peaches = [
...
// G1 x=3,y=2 → 开关1 x=1,y=4
// G2 x=3,y=1 → 开关2 x=3,y=4（G1开后才能到开关2所在位置过双门区域）
const floor_traps = [
    { type: ENTITY_TYPE_ENUM.FLOOR_TRAP, x: 3, y: 2, dir: ENTITY_DIRECTION_ENUM.DOWN, state: ENTITY_STATE_ENUM.ATTACK, entityId: 'l29_ft_1' },
    { type: ENTITY_TYPE_ENUM.FLOOR_TRAP, x: 3, y: 1, dir: ENTITY_DIRECTION_ENUM.DOWN, state: ENTITY_STATE_ENUM.ATTACK, entityId: 'l29_ft_2' },
]
const floor_trap_switches = [
    { type: ENTITY_TYPE_ENUM.FLOOR_TRAP_SWITCH, x: 1, y: 4, dir: ENTITY_DIRECTION_ENUM.DOWN, state: ENTITY_STATE_ENUM.IDLE, entityId: 'l29_ft_1' },
    { type: ENTITY_TYPE_ENUM.FLOOR_TRAP_SWITCH, x: 3, y: 4, dir: ENTITY_DIRECTION_ENUM.DOWN, state: ENTITY_STATE_ENUM.IDLE, entityId: 'l29_ft_2' },
]
